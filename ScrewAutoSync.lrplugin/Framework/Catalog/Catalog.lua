...
function Catalog:setSelectedPhotos( photo, photos )
    ...
    for i, p in ipairs( photos ) do
        if cat:isBuriedInStack( p, bm ) then
            return false, str:fmt( "Unable to select specified photos because at least one of them is buried in a stack (e.g. ^1)", p:getRawMetadata( 'path' ) )
...
function Catalog:getPhotoName( photo, fullPath, rawMeta, fmtMeta )
    ...
    local isVirt = rawMeta and rawMeta[photo] and rawMeta[photo].isVirtualCopy or photo:getRawMetadata( 'isVirtualCopy' )
